...
- **Logo INK4CIPS**: fatto e verificato dal vivo. Logo reale al posto dei placeholder — in `public/logo.png` (login/signup e sidebar workspace, marchio completo con testo). Le icone app (`public/icons/` + `public/apple-touch-icon.png`) usano invece solo il simbolo del pennino isolato (senza testo, illeggibile alle dimensioni di un'icona) a piena tela, con safe-zone maggiorata sulla maskable — vedi Note per i dettagli.
...
- Le icone app (`public/icons/`, `public/apple-touch-icon.png`) sono generate con Pillow isolando solo il simbolo del pennino dal logo sorgente (niente testo "INK4 CIPS", illeggibile a dimensione icona, e niente card/sfondo pre-arrotondato — Apple richiede un'icona "a piena tela" perché iOS applica il proprio arrotondamento sopra) e centrandolo su un canvas alla dimensione target; la variante maskable usa una safe-zone maggiorata (~24% di margine) per non farla ritagliare da Android.
