  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-1 flex-col p-5">
      <div className="mb-5 flex flex-col items-center gap-2.5">
        <img src="/logo.png" alt="INK4CIPS" className="h-[54px] w-[59px] rounded-[11px] object-cover" />
      </div>
      <h1 className="font-heading text-xl font-extrabold text-foreground">Scegli un workspace</h1>
      ...
      <div className="flex flex-col gap-3">
        {workspaces.map((ws) => (
          <div
            key={ws.id}
            className="flex items-center gap-3.5 rounded-2xl border border-[#ececec] p-3.5"
          >
            <Link href={`/w/${ws.slug}`} className="flex min-w-0 flex-1 items-center gap-3.5">
              ...
              <div className="min-w-0">
                <div className="truncate font-heading text-[14.5px] font-bold text-foreground">{ws.name}</div>
            ...
            {(ws.role === "owner" || ws.role === "admin") && (
              <EditWorkspaceSheet
                ...
                trigger={
                  <button
                    type="button"
                    className="flex size-8 shrink-0 items-center justify-center rounded-[9px] bg-ink-soft"
        ...
        <CreateWorkspaceDialog
          trigger={
            <Button
              variant="outline"
              className="mt-1 h-[70px] justify-start gap-3.5 rounded-2xl border-[1.5px] border-dashed border-[#dcdcdc] px-3.5 text-[13.5px] font-semibold text-muted-foreground"
            >
              <div className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-ink-soft">
